...
class AttributeRCNNLossComputation(object):
    ...
    def __call__(self, proposals, attribute_logits, targets=None):
        ...
        no_attr = torch.zeros((len(attributes), 1),
                              device=attributes.device,
                              dtype=torch.float)
        sim_attributes = torch.cat([no_attr, attributes.to(torch.float)], dim=1)
        sum_sim_attributes = sim_attributes.sum(dim=1, keepdim=True)
        sum_sim_attributes[sum_sim_attributes == 0] = 1
        sim_attributes /= sum_sim_attributes
